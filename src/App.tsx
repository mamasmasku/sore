...
`;
    ...
    try {
      ...
      const responseText = response.text
             .replace(/\*\*\*\*\*/g, '---')
             .replace(/^\[([^\]]+)\],/gm, '$1,')   // hapus [ ] di awal baris deskripsi visual
             .replace(/^\[([^\]]+)\]$/gm, '$1');    // hapus [ ] standalone di awal baris
      // Filter: buang semua potongan teks yang tidak mengandung ▶ SEGMEN (teks intro/penjelasan AI)
      const generatedPrompts = responseText.split('---').map(p => p.trim()).filter(p => p.includes('▶ SEGMEN'));
